...
```
  ...
  초기: 모든 OUT[b] = {} = ⊤ (Top, 격자 맨 위)

  1회차: IN₁ = ∪(초기 OUT들) = {} = ⊤
         OUT₁ = GEN ∪ ({} − KILL) = GEN
         GEN ⊇ {} → OUT₁이 {}보다 큰 집합
         ∧=∪에서 큰 집합 = 격자에서 아래 → OUT₁ ≤ ⊤ (아래로 이동!)

  2회차: IN₂ = ∪(OUT₁들)
         OUT₁들이 {}보다 크므로 → IN₂ ⊇ IN₁ (더 큰 집합)
         큰 집합 = 아래 → IN₂ ≤ IN₁ (아래로)
         단조성: IN₂ ≤ IN₁ → f(IN₂) ≤ f(IN₁) → OUT₂ ≤ OUT₁ (출력도 아래로)

  3회차: IN₃ ≤ IN₂ → OUT₃ ≤ OUT₂ (또 아래로)
  ...

  ⊤ ≥ OUT₁ ≥ OUT₂ ≥ OUT₃ ≥ ... ≥ ⊥
  Top에서 시작, 매 반복 아래로만! + 격자 높이 유한 → 반드시 바닥에 도달 → 수렴!

  핵심: ∧=∪에서 "union으로 합치면 집합이 커짐 = 격자에서 내려감"
        "내려가기만 하고 올라가지 않음" + "바닥이 있음" = 수렴 보장
```
