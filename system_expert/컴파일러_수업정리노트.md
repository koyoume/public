...
pre-RA(자유도 높) → RA → post-RA(spill 반영). Integrated(Goodman&Hsu): 레지스터 여유면 병렬성 우선, 부족하면 수명축소.
...
머신: 매 사이클 MEM 1개 + ALU 1개 동시 시작 가능. MEM latency=1cyc. ALU latency=2cyc이지만 파이프라인됨(매 사이클 새 연산 시작 가능, 결과는 2cyc 후). 서로 다른 연산의 ALU가 겹칠 수 있음(throughput=1op/cyc). `A[i]=A[i]*b+c` 루프.
...
여러 반복을 겹쳐서 정상 상태(Kernel)를 만듦. 예제의 Kernel: [Add(i)+Read(i+2)] | [Write(i)+Mul(i+1)] = II=2cyc/iter = 최적! 코드 크기는 원본의 2~3배만 (unrolling 18배 대비 훨씬 적음).

### 5. 파이프라인 루프 구조

Prolog(파이프라인 채움) → Kernel(정상 상태, 매 II사이클 1반복) → Epilog(파이프라인 비움). CPU 파이프라인(fetch/decode/execute/writeback)과 같은 개념을 소프트웨어로.

II(Initiation Interval) = Kernel의 사이클 길이 = 다음 반복 시작까지 간격. II가 작을수록 좋음.

### 6. Do-Across 루프

Do-All 루프: 반복 간 독립. Do-Across 루프: 반복 간 의존성(예: Sum=Sum+A[i]). Do-Across도 파이프라이닝 가능하지만 II가 더 길어질 수 있음.

### 7. Modulo Scheduling — 가장 널리 쓰이는 기법

MII(Minimum Initiation Interval) = max(RMII, PMII).

RMII(Resource MII) = max(ri/Ri). ri=한 반복의 자원 i 사용량, Ri=CPU의 자원 i 수. 예: 2-ALU+1-MEM, ALU 4개+MEM 3개 → RMII=max(4/2, 3/1)=3.

PMII(Precedence MII): 의존성 그래프에서 사이클이 있으면, 각 사이클의 sum(delay)/sum(iteration_diff)의 최대값. 간선 <p,d>: p=반복 간격, d=지연 사이클.

알고리즘: II=MII에서 시작. Modulo Reservation Table(II행×자원열)에 배정 시도. 명령어를 사이클 x에 배정하면 x, x+II, x+2*II,...에서 실행 → x mod II 행의 자원 사용. 성공→확정, 실패→II+1로 재시도. NP-complete → 시행착오.

### 8. Modulo Variable Expansion

어떤 변수의 live range가 II보다 길면 다음 반복에서 같은 레지스터를 덮어씀. 해결: 수명>II인 변수에 여러 레지스터 할당 + 커널 unroll. 예: L(v)=3×II → 레지스터 3개. 또는 Rotating Register(Intel Itanium): HW가 brtop 명령어로 레지스터 번호 자동 시프트 → unroll 없이 해결.

### 9. EPS (Enhanced Pipeline Scheduling)

MS와 완전히 다른 접근. 원래 루프를 유지하면서 DAG 스케줄링을 반복 적용. 명령어가 루프 백엣지를 넘어 이동하면 파이프라이닝 효과 발생. 장점: 임의 제어흐름 루프, 비상수 반복수, outer loop, 비구조적 루프, variable II(경로에 따라 다른 II) 모두 처리 가능.

---

## Ch12. VM, Interpreter, and JIT Compiler — 프로그램을 실행하는 다른 방법들

### 1. 프로그램 실행 방법 3가지 (Ch1 확장)

방법 1: 소스→기계어→HW 실행 (C/C++). 가장 빠르지만 이식성 낮음. 방법 2: 소스→바이트코드(IR)→VM 실행 (Java, Ethereum). 이식성 좋음. 방법 3: 소스→VM이 내부적으로 IR→실행 (JavaScript, Python). 사용자는 소스 직접 실행처럼 보이지만 내부에서 바이트코드 변환.

### 2. Decode-and-Dispatch 인터프리터

가장 단순. while 루프 + switch/case로 바이트코드를 하나씩 해석. 문제: 매 바이트코드마다 간접 분기 2개(switch→case, return→while). CPU 분기 예측기가 잘 동작 못 함 → 느림.

### 3. Threaded Interpretation

switch/return 제거. 각 루틴 끝에서 dispatch table 조회 후 다음 루틴으로 직접 goto. 간접 분기 1개/바이트코드로 줄임 → 50%+ 성능 개선.

### 4. JIT Compilation

바이트코드를 런타임에 기계어로 변환. 핵심: hot 코드만 컴파일 (Hotspot Detection — 함수 호출 횟수 카운트). 컴파일 오버헤드 vs 실행 속도 이득의 트레이드오프. RA: Graph Coloring 대신 Linear Scan (빠르지만 덜 최적). 코드 캐시에 저장 재사용. 서버측 프로그램(수개월~수년 실행)은 컴파일 오버헤드 무시 가능.

### 5. Multi-tier Architecture (JavaScript 엔진)

1st Tier(cold): Parse → 바이트코드 → 인터프리터 (빠른 시작, 최적화 없음). 2nd Tier(warm): Baseline JIT (1:1 변환, 프로파일링 수집). 3rd Tier(hot): Optimizing JIT (프로파일 기반 CSE/DCE/constant folding/RA, 코드 크기 0.32배). Deoptimization: 최적화 가정이 깨지면 baseline으로 되돌아감.

### 6. Profile-based Optimization

JavaScript는 동적 타입 → 컴파일 시점에 타입 모름. 프로파일링으로 "이 변수는 항상 integer" 정보 획득 → 타입 특화 코드 생성(int add만 사용). hidden class 캐싱으로 객체 속성 직접 접근. 가정 깨지면 deoptimization handler로 점프.

---

## Ch13. Compiler-Friendly Programming — 프로그래머가 컴파일러를 도울 수 있다

### 1. 핵심 메시지

같은 빅-O 알고리즘이라도 코드 작성 방식에 따라 10배 성능 차이. 프로그래머가 컴파일러 최적화를 이해하고, 최적화를 방해하는 패턴을 피하면 성능 대폭 향상.

### 2. 프로그래머 LICM — strlen() 문제

느린: for(i=0;i<strlen(s);i++) → strlen() 매 반복 O(N) → 전체 O(N²). 빠른: int len=strlen(s); for(i=0;i<len;i++) → O(N). 왜 컴파일러가 자동으로 못 옮기나? strlen()이 부작용 가능(전역변수 변경 등) → 컴파일러는 함수를 블랙박스로 취급.

### 3. 프로그래머 Strength Reduction

매 반복 n*i 곱셈 → ni=0; ni+=n 덧셈으로 교체. 컴파일러가 자동으로 하기도 하지만(-O1), 복잡한 경우 프로그래머가 직접.

### 4. 프로그래머 CSE

(i-1)*n, (i+1)*n, i*n 곱셈 3번 → inj=i*n+j 계산 후 inj-n, inj+n, inj-1, inj+1로 교체. 곱셈 1번으로 줄임.

### 5. Optimization Blocker #1: 함수 호출

컴파일러는 함수=블랙박스. 부작용 가정 → LICM/CSE 등 적용 제한. 해결: inline 함수, 프로그래머 직접 코드 이동.

### 6. Optimization Blocker #2: Memory Aliasing

a,b가 같은 메모리 가리킬 수 있으면(aliasing) 컴파일러가 b[i]를 지역변수로 교체 못 함. 예: double B[3]=A+3이면 b[i] 쓰기가 a[] 값을 바꿈! 해결: 프로그래머가 지역변수에 누적 후 최종 저장. "컴파일러에게 aliasing 안 한다고 알려주는 것"과 같은 효과.

### 7. 성능 비교

combine1(무최적화): 22.68 → combine1(-O1): 10.12 → combine4(프로그래머 최적화): 1.27. 약 18배 개선. 핵심: vec_length 루프 밖, bounds check 제거, 지역변수 누적, 포인터 직접 접근.
