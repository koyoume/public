...
수렴 증명 (RD, ∧=∪ 기준):
  초기: 모든 OUT[b] = {} = ⊤ (Top, 격자의 맨 위)

  1회차: IN₁ = ∪(초기 OUT들) = {} = ⊤
         OUT₁ = GEN ∪ ({} − KILL) = GEN
         GEN ⊇ {} → OUT₁이 더 큰 집합 → OUT₁ ≤ ⊤ (아래로 이동)
         (∧=∪에서 큰 집합이 아래이므로)

  2회차: IN₂ = ∪(OUT₁들) ⊇ ∪(초기 OUT들) = IN₁
         IN₂가 IN₁보다 크거나 같은 집합 → IN₂ ≤ IN₁ (아래로)
         단조성: IN₂ ≤ IN₁ → f(IN₂) ≤ f(IN₁) → OUT₂ ≤ OUT₁ (아래로)

  3회차: IN₃ ≤ IN₂ → OUT₃ ≤ OUT₂ (또 아래로)
  ...

  ⊤ ≥ OUT₁ ≥ OUT₂ ≥ OUT₃ ≥ ... ≥ ⊥
  (Top에서 시작, 매번 아래로만, 유한 높이 → 반드시 멈춤!)
```
