...
> 삼성 DS System Expert Course | 서울대 김재준 교수 | 최종 업데이트: 2026-04-06
...
**▶ 예시 문제 1 — 동작 영역 판별 및 Ids 계산**

> NMOS: μnCox = 200 μA/V², W/L = 10, Vt = 0.5V
> (a) Vgs=1.5V, Vds=0.5V 일 때 영역과 Ids
> (b) Vgs=1.5V, Vds=1.5V 일 때 영역과 Ids

```
(a) 영역 판별:
    Vgs − Vt = 1.5 − 0.5 = 1.0V
    Vds = 0.5V < 1.0V  →  선형(Linear) 영역

    Ids = μnCox(W/L)[(Vgs−Vt)Vds − Vds²/2]
        = 200×10⁻⁶ × 10 × [1.0×0.5 − 0.5²/2]
        = 2×10⁻³ × [0.5 − 0.125]
        = 2×10⁻³ × 0.375
        = 0.75 mA
...
(b) 영역 판별:
    Vgs − Vt = 1.0V
    Vds = 1.5V > 1.0V  →  포화(Saturation) 영역

    Ids = ½ × μnCox(W/L) × (Vgs−Vt)²
        = ½ × 200×10⁻⁶ × 10 × 1.0²
        = ½ × 2×10⁻³ × 1
        = 1.0 mA
```
...
> 위 NMOS에서 λ=0.1V⁻¹, Vds=2.0V일 때 포화 전류

```
Ids = ½μnCox(W/L)(Vgs−Vt)²(1+λVds)
    = 1.0mA × (1 + 0.1×2.0)
    = 1.0mA × 1.2
    = 1.2 mA

※ λ 없을 때 1.0mA 대비 20% 증가
```

**▶ 예시 문제 3 — 서브스레숄드 스윙 응용**

> S = 70 mV/dec인 트랜지스터에서 Vgs를 Vt에서 −140mV로 낮추면 전류는?

```
전류 변화: ΔVgs/S = 140mV / 70mV per decade = 2 decades
전류는 10² = 100배 감소

즉 OFF 상태 누설은 Vt 기준보다 100배 작아짐
이론 최솟값(60mV/dec)이면: 140/60 ≈ 2.33 → 10^2.33 ≈ 214배 감소
→ S가 작을수록 OFF 상태 누설 억제 효과 큼
```
...
> VOH=4.5V, VOL=0.1V, VIH=3.5V, VIL=1.2V 일 때 NMH, NML, 정상 동작 여부

```
NMH = VOH − VIH = 4.5 − 3.5 = 1.0V
NML = VIL − VOL = 1.2 − 0.1 = 1.1V

정상 동작 조건: NMH > 0 AND NML > 0  →  둘 다 양수 → 정상 동작

※ 만약 VOH < VIH이면 NMH < 0 → HIGH 신호를 다음 게이트가 인식 불가
```

**▶ 예시 문제 2 — 동적 전력 계산 및 VDD 스케일링**

> CL=50fF, f=1GHz, VDD=1.0V 인버터의 동적 전력. VDD를 0.7V로 낮추면?

```
P₁ = CL·VDD²·f = 50×10⁻¹⁵ × 1.0² × 10⁹ = 50 μW

VDD → 0.7V:
P₂ = 50×10⁻¹⁵ × 0.7² × 10⁹ = 50×10⁻¹⁵ × 0.49 × 10⁹ = 24.5 μW

비율: P₂/P₁ = 0.7² / 1.0² = 0.49 → 약 51% 감소

※ 전압을 30% 낮추면 전력은 (0.7)² = 49% 수준으로 감소 (51% 절감)
```

**▶ 예시 문제 3 — 인버터 체인 사이징**

> 입력 Cin=1, 출력 부하 CL=64, 3단 인버터 체인. 각 단 크기와 지연 계산.
> 단위 인버터 지연 td0=10ps, γ=1

```
크기 비율: α = (64/1)^(1/3) = 64^(1/3) = 4

단 1: W = 4×1 = 4
단 2: W = 4×4 = 16
단 3: W = 4×16 = 64  (→ 출력 64 구동)

각 단 지연: td = td0 × (γ + f) = 10ps × (1 + 4) = 50ps
총 지연: 3 × 50ps = 150ps

※ 만약 1단으로 직접 구동한다면:
  td = 10ps × (1 + 64) = 650ps → 4배 이상 느림
```

**▶ 예시 문제 4 — NAND/NOR 사이징 (인버터와 동일 속도)**

> 2입력 NAND 게이트를 인버터(Wp=2, Wn=1)와 동일한 tpHL, tpLH로 만드는 트랜지스터 크기

```
tpHL 동일 조건:
  NAND PDN = NMOS 2개 직렬 → 각 Rn이 2배 → 저항 보상 위해 W를 2배
  → NMOS W = 2

tpLH 동일 조건:
  NAND PUN = PMOS 2개 병렬 → 저항 절반 → 보상 불필요
  → PMOS W = 2  (인버터 PMOS와 동일)

결론: PMOS W=2, NMOS W=2

※ NOR2의 경우:
  PDN = NMOS 2개 병렬 → NMOS W=1 유지
  PUN = PMOS 2개 직렬 + PMOS이동도 절반 → PMOS W = 2×2 = 4
```
...
**▶ 예시 문제 1 — Path Effort 및 최적 사이징 (단계별 풀이)**

> 경로: Cin=1 → [NAND2] → [NOR2] → [INV] → CL=32
> td0=10ps, γ=1. 최소 지연과 각 게이트 크기를 구하라.

```
[Step 1] 각 게이트 LE, p 확인
  NAND2: LE=4/3, p=2
  NOR2:  LE=5/3, p=2
  INV:   LE=1,   p=1

[Step 2] Path 파라미터 계산
  G  = (4/3) × (5/3) × 1 = 20/9 ≈ 2.22
  B  = 1 (분기 없음)
  EF = CL/Cin = 32/1 = 32
  PE = G × B × EF = 2.22 × 32 = 71.1
  P  = 2 + 2 + 1 = 5 (기생 지연 합)

[Step 3] 최적 단수 확인
  N = log₄(PE) = log₄(71.1) ≈ 3.1  → N=3단 적절

[Step 4] 최소 지연 계산
  D = N × td0 × PE^(1/N) + P × td0
    = 3 × 10ps × 71.1^(1/3) + 5 × 10ps
    = 3 × 10ps × 4.14 + 50ps
    = 124.2ps + 50ps = 174.2ps

[Step 5] 단당 최적 effort
  f̂ = PE^(1/N) = 71.1^(1/3) ≈ 4.14

[Step 6] 뒤에서 앞으로 크기 결정
  INV:   Cin₃ = CL × LE₃ / f̂ = 32 × 1 / 4.14 ≈ 7.7
  NOR2:  Cin₂ = Cin₃ × LE₂ / f̂ = 7.7 × (5/3) / 4.14 ≈ 3.1
  NAND2: Cin₁ = Cin₂ × LE₁ / f̂ = 3.1 × (4/3) / 4.14 ≈ 1.0 ✓ (입력과 일치)
```

**▶ 예시 문제 2 — 분기 노력 포함**

> 위 경로에서 NOR2 출력이 on-path(CL=32)와 off-path(Coff=16)로 분기될 때 PE 재계산

```
b = (Con + Coff) / Con = (32 + 16) / 32 = 1.5
B = b = 1.5  (분기가 1곳)

PE_new = G × B × EF = 2.22 × 1.5 × 32 = 106.7
D_new = 3 × 10 × 106.7^(1/3) + 50 = 3×10×4.74 + 50 = 142.2+50 = 192.2ps

※ 분기로 인해 지연이 174ps → 192ps로 약 10% 증가
```
...
> Cu 배선: ρ=1.7μΩ·cm, W=0.1μm, H=0.2μm, L=100μm

```
R□ = ρ/H = (1.7×10⁻⁶ Ω·cm) / (0.2×10⁻⁴ cm)
   = 1.7×10⁻⁶ / 2×10⁻⁵ = 0.085 Ω/□

정사각형 수 = L/W = 100μm / 0.1μm = 1000개

R = R□ × (L/W) = 0.085 × 1000 = 85 Ω
```

**▶ 예시 문제 2 — Elmore 지연 계산**

> RC 트리: 소스→R1(100Ω)→노드1(C1=10fF)→R2(200Ω)→노드2(C2=20fF)
>                                            └→R3(150Ω)→노드3(C3=15fF)
> 노드2까지의 Elmore 지연을 구하라.

```
노드2까지의 경로: 소스→R1→R2→노드2

각 노드의 공통 경로 저항(R_k2):
  노드1: 소스→노드1 경로와 소스→노드2 경로의 공통 = R1 = 100Ω
  노드2: 소스→노드2 경로와 소스→노드2 경로의 공통 = R1+R2 = 300Ω
  노드3: 소스→노드3 경로(R1+R3)와 소스→노드2 경로(R1+R2)의 공통 = R1 = 100Ω

TD2 = R1·2·C1 + (R1+R2)·C2 + R1·C3
    = 100×10×10⁻¹⁵ + 300×20×10⁻¹⁵ + 100×15×10⁻¹⁵
    = 1ps + 6ps + 1.5ps = 8.5ps

※ 소스에 가까운 R1이 모든 커패시턴스(C1+C2+C3)에 영향을 주므로
  R1·(C1+C2+C3) = 100×45×10⁻¹⁵ = 4.5ps 가 지배적
```

**▶ 예시 문제 3 — 리피터 최적화**

> 배선: Rw=1kΩ, Cw=500fF. 단위 인버터: Rinv=10kΩ, Cinv=5fF

```
Wopt = √(Rinv·Cw / Rw·Cinv)
     = √(10×10³ × 500×10⁻¹⁵ / 1×10³ × 5×10⁻¹⁵)
     = √(5×10⁻⁹ / 5×10⁻¹²)
     = √1000 ≈ 31.6  → W ≈ 32배 크기 인버터 사용

Nopt = √(Rw·Cw / 2Rinv·Cinv)
     = √(1×10³ × 500×10⁻¹⁵ / 2×10×10³ × 5×10⁻¹⁵)
     = √(5×10⁻¹⁰ / 10⁻¹⁰)
     = √5 ≈ 2.2  → N=2개 리피터 삽입

리피터 없을 때 지연: Rw·Cw/2 = 1kΩ×500fF/2 = 250ps
리피터 삽입 후 지연: 대폭 감소 (L²→L 비례)
```
...
**▶ 예시 문제 1 — 최대 클럭 주파수 계산**

> tpcq=0.3ns, tsetup=0.2ns, tpd(조합논리)=1.5ns. 최대 동작 주파수?
> 클럭 스큐 tskew=0.1ns가 추가되면?

```
스큐 없을 때:
  Tc ≥ tpcq + tpd + tsetup = 0.3 + 1.5 + 0.2 = 2.0ns
  fmax = 1/Tc = 1/2.0ns = 500 MHz

스큐 0.1ns 추가:
  Tc ≥ tpcq + tpd + tsetup + tskew = 2.0 + 0.1 = 2.1ns
  fmax = 1/2.1ns ≈ 476 MHz  (약 5% 감소)
```

**▶ 예시 문제 2 — Hold Time 위반 검사**

> tccq=0.1ns, thold=0.3ns, tcd(조합논리)=0.15ns, tskew=0.05ns
> Hold Time 위반 여부?

```
스큐 없을 때 조건: tcd + tccq ≥ thold
  0.15 + 0.1 = 0.25ns  <  0.3ns  →  ❌ Hold Time 위반!

스큐 포함 조건: tcd + tccq ≥ thold + tskew
  0.25ns  <  0.3 + 0.05 = 0.35ns  →  더욱 위반

해결책: 조합 논리 경로에 버퍼 삽입하여 tcd 증가
  필요 tcd ≥ thold + tskew − tccq = 0.35 − 0.1 = 0.25ns
```

**▶ 예시 문제 3 — Pulsed Latch Time Borrowing**

> FF 기반: Tc=2ns, tpcq=0.3ns, tsetup=0.2ns → 최대 tpd=1.5ns
> Pulsed Latch(Tpw=0.4ns)로 교체하면 최대 tpd는?

```
FF 최대 tpd: Tc − tpcq − tsetup = 2.0 − 0.3 − 0.2 = 1.5ns

Pulsed Latch 최대 tpd:
  tpd ≤ Tc − tpcq − tsetup + Tpw = 1.5 + 0.4 = 1.9ns

→ 0.4ns(26%) 추가 허용 → 더 복잡한 조합 논리 수용 가능

단, Hold Time 조건은 강화:
  tcd ≥ thold − tccq + Tpw  (Tpw만큼 더 커야 함)
```
...
> 다이나믹 게이트: VDD=1V, C2(출력노드)=20fF, C1(내부노드)=10fF
> 내부 노드 프리차지 없을 때, A=1, B=0이면 출력 전압은?

```
전하 보존: C2·VDD + C1·0 = (C1+C2)·V(X2)
...
→ 원래 VDD(1V)에서 0.667V로 강하!
→ 인버터 트립 전압이 0.5V라면 출력이 틀려지진 않지만
  트립 전압이 0.7V라면 출력 오류 발생

해결: C1도 VDD로 프리차지 → V(X2) = VDD × C2/(C2+C2) = VDD (유지)
```

**▶ 예시 문제 2 — 도미노 vs 정적 CMOS 전력 비교**

> 동일 기능의 인버터: 정적 CMOS vs 도미노 로직. 스위칭 활동(SW) 비교
...
도미노 인버터 (A=1 고정):
  매 클럭마다 프리차지 트랜지스터 스위칭:
    CLK=0: 출력 노드 VDD로 충전 (1←0)
    CLK=1: A=1이면 즉시 방전 (0→1... 다시 프리차지)
  → 매 사이클 1회 스위칭 → SW=1
  전력 = CL·VDD²·f > 정적 CMOS

결론: 입력이 대부분 1인 경우 도미노가 훨씬 전력 소모 큼
```
...
**▶ 예시 문제 1 — DRAM 읽기 전압 변화 계산**

> Ccell=30fF, Cbl=600fF, VDD=1.8V, BL 프리차지=VDD/2=0.9V
> "1" 읽기 시 ΔVbl과 BL 최종 전압은?

```
ΔVbl("1") = +Ccell·VDD/2 / (Cbl + Ccell)
           = +30fF × 0.9V / (600fF + 30fF)
           = +27 / 630
           = +0.0429V ≈ +43mV

BL 최종 전압 = 0.9V + 0.043V = 0.943V

"0" 읽기 시: ΔVbl = −43mV → BL = 0.857V

센스 앰프 감지 차이: 0.943V − 0.857V = 86mV
→ 이 작은 차이를 VDD 또는 GND로 증폭하는 것이 센스 앰프의 역할

Ccell/Cbl 비율이 클수록 ΔVbl 커짐 → DRAM 설계에서 Ccell 확보가 중요
```

**▶ 예시 문제 2 — Flash 읽기 동작 판별**

> NAND Flash 4셀 스트링. WL[1]의 셀을 읽으려 한다.
> 각 WL에 인가할 전압과 판독 원리를 설명하라.

```
WL[0], WL[2], WL[3]: Vpass = 4~5V  → 데이터와 무관하게 강제로 ON
WL[1]:               Vread = 0V     → 이 셀의 Vth에 따라 ON/OFF 결정

판독:
  셀 데이터 "1": FG에 전자 없음 → Vth 낮음(예: -1V)
                 Vread(0V) > Vth(-1V) → 셀 ON → 전류 흐름 → BL 방전 → "1"

  셀 데이터 "0": FG에 전자 있음 → Vth 높음(예: 2V)
                 Vread(0V) < Vth(2V) → 셀 OFF → 전류 없음 → BL 유지 → "0"

※ Vpass는 모든 셀의 가능한 Vth 최댓값보다 높아야 함
```
...
**▶ 예시 문제 1 — SRAM 읽기 안정성 분석**

> 6T SRAM: Q=0 저장. 읽기 중 BL=VDD, WL=1이 켜짐.
> NR=2k, XR=4k, PR=8k(Ω 단위 ON 저항)일 때 Q 노드 전압과 위험 여부를 분석하라.
> (인버터 PL-NL 트립 전압=0.4V)

```
읽기 중 Q 노드 전압 (전압 분배):
  BL(VDD=1V) → XR → Q 노드 → NR → GND

  전류 경로: VDD/(XR + NR) = 1V/(4k + 2k) = 0.167mA
  Q 전압 = NR/(XR+NR) × VDD = (2k/6k) × 1V = 0.333V

Q=0.333V < 트립 전압(0.4V) → 안전 ✅

만약 XR을 2kΩ으로 줄이면(XR=NR):
  Q = NR/(XR+NR) × VDD = (2k/4k) × 1V = 0.5V > 0.4V → ❌ 불안정!

결론: NR < XR 조건이 읽기 안정성의 핵심
      NR:XR = 2:1 비율이 이를 보장
```

**▶ 예시 문제 2 — 쓰기 성공 조건 분석**

> 6T SRAM: Q=1 저장 상태에서 "0"을 쓰려 한다.
> BL=0, BLb=1, WL=1. XL=2kΩ, PL=8kΩ (ON 저항)
> 쓰기 성공 여부와 Q 노드 전압?

```
쓰기 중 Q 노드:
  VDD(1V) → PL → Q 노드 → XL → GND

  Q 전압 = XL/(PL+XL) × VDD = (2k/10k) × 1V = 0.2V

  Q가 0.2V로 내려감 → 오른쪽 인버터(PR+NR)가
  Qb를 1로 올림 → 피드백으로 Q가 더 내려감 → 쓰기 성공 ✅

만약 XL=PL=8kΩ이면:
  Q = 8k/16k × 1V = 0.5V = 트립 전압 경계 → 불안정

결론: XL < PL 조건(즉 XL이 PL보다 강해야)이 쓰기 성공 조건
```

**▶ 예시 문제 3 — 프리디코딩 게이트 수 절감**

> 4비트 주소(16개 WL) 디코더를 직접 디코딩 vs 프리디코딩 비교

```
직접 디코딩:
  16개 × 4입력 AND = 64개 게이트 입력 (트랜지스터 ∝ 128개)

프리디코딩 (2+2 분할):
  A[1:0] → 1-of-4 프리디코더: 2입력 AND 4개 = 8개 게이트 입력
  A[3:2] → 1-of-4 프리디코더: 2입력 AND 4개 = 8개 게이트 입력
  최종 단: 2입력 AND 16개 = 32개 게이트 입력
  합계: 8 + 8 + 32 = 48개 (직접 대비 25% 절감)

속도 측면:
  직접: 4입력 AND → 느림 (LE 큼)
  프리디코딩: 2입력 AND만 → 빠름 (LE 작음)
```
